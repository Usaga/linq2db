...
namespace LinqToDB.Tools
{
	...
	internal static class NameNormalizationTest
	{
		...
		{
			ILanguageServices languageServices = new CSharpLanguageServices();
			var typeParser = new TypeParser(languageServices);

			var codeModel = new CodeBuilder(languageServices, typeParser);
			var modelFile = codeModel.File("TestNormalization");
			...
			// name
			modelFile.Imports.Add(codeModel.Import(new[] { new CodeIdentifier("System"), new CodeIdentifier("Linq") }));
			modelFile.Imports.Add(codeModel.Import(new[] { new CodeIdentifier("LinqToDB") }));
			modelFile.Imports.Add(codeModel.Import(new[] { new CodeIdentifier("Tests") }));
			...
			var knownTypes = new TypeCollectorVisitor(languageServices);
			var namesNormalization = new CSharpNameNormalizationVisitor(languageServices, knownTypes.LocalTypes, knownTypes.ExternalTypes);
			namesNormalization.Visit(modelFile);
			var settings = new CodeGenerationSettings();
			var namespaces = new CollectAllNamespacesVisitor(languageServices);
			namespaces.Visit(modelFile);

			var nameScopes = new ScopedNamesCollector(languageServices);
			nameScopes.Visit(modelFile);

			CodeGenerationVisitor codeGenerator = new CSharpCodeGenerator(typeParser, languageServices, settings, namespaces.Namespaces, nameScopes.ScopedNames, nameScopes.KnownTypes);
			codeGenerator.Visit(modelFile);
			File.WriteAllText(@"..\..\..\Generated\Test.cs", codeGenerator.GetResult());
		}

		private static CodeElementNamespace CreateNamespace(CodeBuilder codeModel, bool final, string name1, string name2, bool withEmptyClasses)
		{
		...
		private static CodeBlockBuilder CreateBody(CodeBuilder codeModel, CodeBlockBuilder block, int nestingLevel, string name1, string name2)
		{
		...
		private static void CreateOverloadMethod(CodeBuilder codeModel, ClassBuilder builder, bool isStatic, Direction direction, string name)
		{
