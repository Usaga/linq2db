...
namespace LinqToDB.Tools
{
	internal static class Program
	{
		...
		private static void BuildModel(string configName)
		{
			...
			{
				var schemaProvider = new CodeGen.Schema.SchemaProvider(dc, schemaSettings, typeParser);
				var namingServices = new NamingServices(languageServices, nameConverter);

				var modelBuilder = new ModelBuilder(schemaProvider, schemaProvider, languageServices, typeParser, schemaSettings, codegenSettings, contextSettings, nameConverter, namingServices);
				var dataModel = modelBuilder.Build();

				var sqlBuilder = dc.DataProvider.CreateSqlBuilder(dc.MappingSchema);
				var builder = new CodeBuilder(languageServices, typeParser);
				var metadata = new AttributeBasedMetadataBuilder(builder, sqlBuilder);

				var codeBuilder = new CodeModelBuilder(codegenSettings, nameConverter, languageServices, typeParser, metadata, builder, namingServices, sqlBuilder);
				...
				for (var i = 0; i < files.Length; i++)
				{
					...
					File.WriteAllText($@"{root}\{files[i].FileName}.{languageServices.FileExtension}", source[i]);
				}
			}
