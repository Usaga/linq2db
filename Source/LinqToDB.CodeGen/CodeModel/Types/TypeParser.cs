...
namespace LinqToDB.CodeGen.CodeModel
{
	public class TypeParser : ITypeParser
	{
		...
		public TypeParser(ILanguageServices languageServices)
		{
		...
		{
			var (type, length) = ParseTypeInternal(null, typeName, valueType, false, false);
		...
		private (IType? type, int length) ParseTypeInternal(IType? parentType, string typeName, bool valueType, bool nested, bool genericList)
		{
			...
			foreach (var (chr, cat) in typeName.EnumerateCharacters())
			{
				...
				else if (chr == "<")
				{
					switch (state)
					{
						...
						case ParserState.ParseTypeNameOrParentTypeName:
							...
							var (typeArg, len) = ParseTypeInternal(null, typeName.Substring(length), valueType, false, true);
							if (typeArg == null)
							{
								openGeneric = true;
								openGenericCount = 1;
				...
				else
				{
					...
					{
						case ParserState.SearchForIdentifierStart:
							if (!_languageServices.IsValidIdentifierFirstChar(chr, cat))
								throw new InvalidOperationException();
						...
						case ParserState.ParseNamespaceTypeNameOrParentTypeName:
						case ParserState.ParseTypeNameOrParentTypeName:
							if (!_languageServices.IsValidIdentifierNonFirstChar(chr, cat))
								throw new InvalidOperationException();
							currentIdentifier.Append(chr);
							continue;
			...
			else
			{
				...
				{
					var alias = _languageServices.GetAlias(nsArr, name!.Name);
					parsedType = new RegularType(
							nsArr,
							name,
							false,
			...
			if (isParent)
			{
				return ParseTypeInternal(parsedType, typeName.Substring(length), valueType, false, genericList);
			}
		...
		private IType ParseCLRType(Type type)
		{
			...
			if (type.IsArray)
			{
				...
				return new ArrayType(
					ParseCLRType(type.GetElementType()),
					new int?[rank],
					false);
			}
			...
			var alias = _languageServices.GetAlias(ns, type.Name);
