...
#pragma warning restore IDE0082 // 'typeof' can be converted  to 'nameof'
		...
		public CSharpLanguageServices()
		{
		}

		string? ILanguageServices.MissingXmlCommentWarnCode => "1591";
		bool ILanguageServices.SupportsNRT => true;
		string ILanguageServices.FileExtension => "cs";
		...
		CodeModelVisitor ILanguageServices.GetNamesNormalizer()
		{
			return new CSharpNameNormalizationVisitor(this, new HashSet<IType>(), new HashSet<IType>());
		}

		CodeGenerationVisitor ILanguageServices.GetCodeGenerator(CodeGenerationSettings settings, ISet<CodeIdentifier[]> namespaces, ITypeParser typeParser, IReadOnlyDictionary<string, ISet<string>> scopedNames, IReadOnlyDictionary<string, ISet<string>> knownTypes)
		{
			return new CSharpCodeGenerator(typeParser, this, settings, namespaces, scopedNames, knownTypes);
		}
	}
}
