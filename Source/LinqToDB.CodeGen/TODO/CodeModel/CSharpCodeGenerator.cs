...
namespace LinqToDB.CodeGen.CodeModel
{
	...
	public class CSharpCodeGenerator : CodeGenerationVisitor
	{
		...
		private readonly IReadOnlyDictionary<string, ISet<string>> _scopedNames;
		private readonly IReadOnlyDictionary<string, ISet<string>> _knownTypes;
		private List<CodeIdentifier> _currentScope = new();

		public CSharpCodeGenerator(ITypeParser typeParser, ILanguageServices langServices, CodeGenerationSettings settings, ISet<CodeIdentifier[]> knownNamespaces, IReadOnlyDictionary<string, ISet<string>> scopedNames, IReadOnlyDictionary<string, ISet<string>> knownTypes)
			: base(settings)
		{
		...
		private List<CodeElementImport>? _currentImports;
		...
		private void WriteProperties(List<CodeProperty> properties)
		{
			...
			for (var i = 0; i < properties.Count; i++)
			{
				...
				initializer[i] = BuildFragment<CSharpCodeGenerator>(b =>
				{
			...
			for (var i = 0; i < properties.Count; i++)
			{
				...
				WriteWithPadding(initializer[i], maxInitializer);
		...
		private void WriteLiteral(object? value, IType type, bool targetTyped)
		{
			...
			else if (value.GetType().IsEnum)
			{
				...
				else
				{
					...
					WriteLiteral(Convert.ChangeType(value, underlyingType), _typeParser.Parse(underlyingType), true);
				}
			}
			else
		...
		protected override void Visit(TypeInitializer cctor)
		{
			...
			Write("()");
			WriteMethodBody(cctor.Body!, false, true, false, true);
		}
		...
		protected override void Visit(CodeClass @class)
		{
			...
			// change scope after custom attributes!
		...
		private void RenderType(IType type, CodeIdentifier? nameOverride, bool ignoreCurrentType, bool typeOnlyContext)
		{
			...
			{
				var needsNamespace = false;
				var name = nameOverride?.Name ?? type.Name!.Name;

				// type with same name in multipe namespaces
				if (_knownTypes.TryGetValue(name, out var namespaces))
					needsNamespace = namespaces.Count > 1 || !namespaces.Contains(string.Join(".", type.Namespace.Select(_ => _.Name)));
				...
				if (!needsNamespace)
				{
					...
					{
						if (type.Namespace.SequenceEqual(_currentScope.Take(i), CodeIdentifierComparer.Instance) && typeOnlyContext)
						{
							// type from current scope, do nothing
							break;
						}

						var currentScope = string.Join(".", _currentScope.Select(_ => _.Name));

						if (_scopedNames[currentScope].Contains(name) || (!typeOnlyContext && comparer.Equals(_currentScope[i].Name, name)))
						{
							// name conflict
							needsNamespace = true;
							break;
				...
				if (!needsNamespace)
				{
					if (_scopedNames.TryGetValue(string.Empty, out var names) && names.Contains(name))
					{
						needsNamespace = true;
					}
					else if (_currentImports != null)
					{
						...
						{
							if (!type.Namespace.SequenceEqual(import.Parts, CodeIdentifierComparer.Instance))
							{
								var scope = string.Join(".", import.Parts.Select(_ => _.Name));
								if (_scopedNames.TryGetValue(scope, out names) && names.Contains(name))
								{
									needsNamespace = true;
									break;
								}
				...
				case TypeKind.Array:
				{
					...
					foreach (var size in arrType.ArraySizes)
					{
						if (first)
							...
							Write(',');
						if (size != null)
							WriteLiteral(size.Value, _typeParser.Parse<int>(), false);
					}
					Write(']');
					break;
				}
