...
namespace LinqToDB.CodeGen.CodeModel
{
	public class CodeBuilder
	{
		...
		public CodeBuilder(ILanguageServices langServices, ITypeParser typeParser)
		{
		...
		public NamespaceBuilder Namespace(CodeIdentifier[] name)
		{
			return new NamespaceBuilder(new CodeElementNamespace(name));
		}
		...
		{
			return _typeParser.Parse(type).WithNullability(nullable);
		}
		...
		{
			var type = _typeParser.Parse(typeName, valueType);
		...
		{
			return new CodeConstant(type, (object?)null, targetTyped);
		}
		...
		public CodeConstant Constant(Func<string> delayedValue, bool targetTyped)
		{
			return new CodeConstant(Type(typeof(string), false), delayedValue, targetTyped);
		}
