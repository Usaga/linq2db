...
namespace LinqToDB.CodeGen.CodeModel
{
	public class CSharpNameNormalizationVisitor : NoopCodeModelVisitor
	{
		...
		{
			// actually variables only in this case
			_parametersAndVariables = _langServices.GetUniqueNameCollection();
			base.Visit(cctor);
			_parametersAndVariables = null;
		}
		...
		private string FixName(ISet<string> scopeNames, CodeIdentifier name, Func<string, string> getFullName, bool add)
		{
			...
			var basicName = _langServices.NormalizeIdentifier(name.Name);
			var finalName = basicName;

			var fullName = getFullName(basicName);

			var cnt = 0;
			while (fullName == "" || scopeNames.Contains(fullName))
			{
				//if (name.Protected)
				...
				if (name.FixOptions != null)
				{
					switch (name.FixOptions.FixType)
					{
						case FixType.Replace:
							basicName = name.FixOptions.Fixer;
							break;
						case FixType.ReplaceWithPosition:
							basicName = name.FixOptions.Fixer + name.Position?.ToString(NumberFormatInfo.InvariantInfo);
							break;
						case FixType.Suffix:
							basicName = name.Name + name.FixOptions.Fixer;
							break;
						case FixType.SuffixWithPosition:
							basicName = name.Name + name.FixOptions.Fixer + name.Position?.ToString(NumberFormatInfo.InvariantInfo);
							break;
						default:
							throw new InvalidOperationException();
					}
				...
				finalName = string.IsNullOrEmpty(basicName) ? "_" : basicName;

				if (cnt > 0)
					finalName += cnt.ToString(NumberFormatInfo.InvariantInfo);

				finalName = _langServices.NormalizeIdentifier(finalName!);

				fullName = getFullName(finalName!);
				cnt++;
			}
