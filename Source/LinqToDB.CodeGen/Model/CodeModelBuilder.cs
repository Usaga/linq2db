	...
	public class CodeModelBuilder
	{
			...
			ISqlBuilder sqlBuilder)
		{
			...
			foreach (var (typeName, isValueType) in settings.NonBooleanEqualityTypes)
				_nonBooleanEqualityTypes.Add(_typeParser.Parse(typeName, isValueType));

			_disableXmlWarn = _languageServices.MissingXmlCommentWarnCode != null && _settings.SuppressMissingXmlCommentWarnings;
			_nrtEnabled = _languageServices.SupportsNRT && _settings.NullableReferenceTypes;
		...
		public string[] GetSourceCode(CodeFile[] files)
		{
			...
			var namesNormalize = _languageServices.GetNamesNormalizer();
			foreach (var file in files)
				namesNormalize.Visit(file);

			var namespacesCollector = new CollectAllNamespacesVisitor(_languageServices);

			var importsCollector = new NamespaceCollectorVisitor(_languageServices);

			var nameScopes = new ScopedNamesCollector(_languageServices);
			foreach (var file in files)
			...
			for (var i = 0; i < files.Length; i++)
			{
				...
				namespacesCollector.Namespaces.Clear();
				namespacesCollector.Visit(file);

				//foreach (var ns in _settings.ConflictingNamespaces)
				//{
				//	// TODO: HACK: add separate method for namespace parsing as type parser allows too much
				//	var type = _typeParser.Parse(ns, false);
				//	var nsName = new CodeIdentifier[type.Namespace?.Length ?? 0 + 1];
				//	nsName[nsName.Length - 1] = type.Name!;
				//	if (type.Namespace != null)
				//	{
				//		for (var j = 0; j < type.Namespace.Length; j++)
				//			nsName[j] = type.Namespace[j];
				//	}

				//	namespacesCollector.Namespaces.Add(nsName);
				//}

				importsCollector.Namespaces.Clear();
				importsCollector.Visit(file);

				foreach (var import in importsCollector.Namespaces.OrderBy(_ => _, CodeIdentifierComparer.Instance))
				...
				foreach (var name in _settings.ConflictingNames)
				{
					...
					var parsedName = _typeParser.Parse(name, false);
					if (parsedName is RegularType type && type.Parent == null)
					{
						...
						if (!nameScopes.ScopedNames.TryGetValue(scope, out var names))
							nameScopes.ScopedNames.Add(scope, names = new HashSet<string>(_languageServices.GetNameComparer()));
				...
				var codeGenerator = _languageServices.GetCodeGenerator(_settings, namespacesCollector.Namespaces, _typeParser, nameScopes.ScopedNames, nameScopes.KnownTypes);
		...
		private CodeFile CreateFile(DataModel dataModel, string fileName)
		{
			...
			if (_disableXmlWarn)
				file.Add(_builder.DisableWarnings(_languageServices.MissingXmlCommentWarnCode!));
		...
		private void BuildDataContext(CodeFile contextFile, DataModel dataModel, List<CodeFile> files)
		{
			...
			var contextIsDataConnection = contextBuilder.Type.Inherits != null && TypeComparer.IgnoreNRT.Equals(contextBuilder.Type.Inherits.Type, _typeParser.Parse<DataConnection>());
		...
		{
			var ctors = new List<CodeBlockBuilder>();
			...
			{
				var configurationParam = _builder.Parameter(_builder.Type(typeof(string), false), _builder.Identifier("configuration"), Direction.In);
			...
			{
				var optionsParam = _builder.Parameter(_builder.Type(typeof(LinqToDbConnectionOptions), false), _builder.Identifier("options"), Direction.In);
				ctors.Add(contextConstructors.New()
			...
			{
				var typedOptionsParam = _builder.Parameter(_builder.Type(typeof(LinqToDbConnectionOptions<>), false, contextBuilder.Type.Type), _builder.Identifier("options"), Direction.In);
				ctors.Add(contextConstructors.New()
		...
		private CodeBlockBuilder? _schemaInitializer;
		private CodeIdentifier? _schemaProperty;
		private PropertyGroup? _staticContextProperties;

		private (CodeBlockBuilder initializer, CodeIdentifier schemaProperty) GetSchemaInitializer(ClassBuilder dataContextBuilder)
		{
			if (_schemaInitializer == null)
			{
				var schemaProp = _staticContextProperties!.New(_builder.Identifier("ContextSchema"), _builder.Type(typeof(MappingSchema), false))
					...
					.Static()
					.Default(false)
					.SetValue(_builder.New(_builder.Type(typeof(MappingSchema),false), Array.Empty<ICodeExpression>(), Array.Empty<AssignExpression>()));
			...
			bool backReference)
		{
			...
			var thisParam = _builder.Parameter(thisEntity.Type.Type, _builder.Identifier("obj"), Direction.In);
			var ctxParam = _builder.Parameter(_builder.Type(typeof(IDataContext), false), _builder.Identifier("db"), Direction.In);
			...
			{
				methodBuilder.Body().Append(_builder.Throw(_builder.New(_typeParser.Parse<InvalidOperationException>(), new ICodeExpression[] { _builder.Constant("Association cannot be called outside of query", true) }, Array.Empty<AssignExpression>())));
			}
			else
			{
		...
		private void BuildExtraSchema(
			...
			CodeBlockBuilder initMethodBody,
			ExplicitSchemaModel extraSchema)
		{
			...
			initMethodBody.Append(
				_builder.Assign(
					...
					_builder.New(contextClass.Type.Type, new ICodeExpression[] { _builder.This() }, Array.Empty<AssignExpression>())));
			...
			var ctorParam = _builder.Parameter(_builder.Type(typeof(IDataContext), false), _builder.Identifier("dataContext"), Direction.In);
		...
		private void BuildAggregateFunctions(List<AggregateFunctionModel> aggregateFunctions, RegionGroup functionsGroup)
		{
			foreach (var func in aggregateFunctions)
			{
				...
				var body = method.Body().Append(
					_builder.Throw(_builder.New(_builder.Type(typeof(InvalidOperationException), false), Array.Empty<ICodeExpression>(), Array.Empty<AssignExpression>())));
				...
				var sourceParam = _builder.Parameter(_builder.Type(typeof(IEnumerable<>), false, new[] { source }), _builder.Identifier("src"), Direction.In);
				...
				if (func.Parameters.Count > 0)
				{
					...
					for (var i = 0; i < func.Parameters.Count; i++)
					{
						...
						var p = _builder.Parameter(parameterType, _builder.Identifier(param.Parameter.Name, null/*ctxModel.Parameter*/, i + 1), Direction.In);
						method.Parameter(p);
		...
		private void BuildTableFunctions(List<TableFunctionModel> tableFunctions, RegionGroup functionsGroup, ClassBuilder ownerType)
		{
			foreach (var func in tableFunctions)
			{
				...
				methodInfo.AddSetter(_builder.Call(
						new TypeReference(_builder.Type(typeof(MemberHelper), false)),
			...
			ClassBuilder dataContextBuilder)
		{
			foreach (var func in scalarFunctions)
			{
				...
				else
				{
					...
					var initializers = new AssignExpression[func.ReturnTuple.Fields.Count];
			...
			bool contextIsDataConnection)
		{
			foreach (var proc in storedProcedures)
			{
				...
				var ctxParam = _builder.Parameter(
					// see method notes
					contextIsDataConnection ? dataContext.Type.Type : _typeParser.Parse<DataConnection>(),
				...
				else
				{
					...
					if (ordinalMapping)
					{
						...
						var initializers = new AssignExpression[customTable!.Columns.Count];
						...
						for (var i = 0; i < customTable.Columns.Count; i++)
						{
							...
							initializers[i] = _builder.Assign(
								prop.Name,
								_builder.Call(
									new TypeReference(_builder.Type(typeof(Converter), false)),
			...
			int idx)
		{
			...
			ctorParams.Add(parameter.Direction == Direction.In || parameter.Direction == Direction.Ref ? parameter.Name : _builder.Null(_builder.Type(typeof(object), true), true));
			if (dataType != null)
				ctorParams.Add(_builder.Constant(dataType.Value, true));

			if (parameter.Direction == Direction.Out && !returnParameter)
			...
			else if (parameter.Direction == Direction.Ref)
			...
			MethodGroup findMethodsGroup)
		{
			...
			methodParameters.Add(_builder.Parameter(_builder.Type(typeof(ITable<>), false, entityType), _builder.Identifier("table"), Direction.In));
			...
			foreach (var column in pks)
			{
				...
				var parameter = _builder.Parameter(column.Property.Type, _builder.Identifier(paramName), Direction.In);
			...
			find
				.Public()
				.Extension()
				.Returns(_builder.Type(entityType, true))
				.Body()
				.Append(
					_builder.Return(
						_builder.ExtCall(
		...
		private ICodeExpression CorrectEquality(IType type, CodeBinaryExpression expr)
		{
			if (expr.Operation == BinaryOperation.Equal && _nonBooleanEqualityTypes.Contains(type))
				return _builder.Cast(_typeParser.Parse<bool>(), expr);
