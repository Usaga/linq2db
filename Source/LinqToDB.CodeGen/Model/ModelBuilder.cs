...
Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.";
			...
			{
				baseEntityType = _typeParser.Parse(_contextSettings.BaseEntityClass, false);
			}
		...
		private void BuildAggregateFunction(DataModel model, AggregateFunction func, ISet<string> defaultSchemas)
		{
			...
			var funcModel = new AggregateFunctionModel(name, method, metadata, (typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(scalarResult.Nullable));
		...
		private void BuildScalarFunction(DataModel model, ScalarFunction func, ISet<string> defaultSchemas)
		{
			...
			switch (func.Result.Kind)
			{
				...
				case ResultKind.Scalar:
				{
					...
					funcModel.Return = (typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(scalarResult.Nullable);
					break;
				}
				case ResultKind.Tuple:
				{
					...
					foreach (var field in tupleResult.Fields)
					{
						...
						var prop = new PropertyModel(_namingServices.NormalizeIdentifier(_contextSettings.FunctionTupleResultPropertyName, field.Name ?? "Field"), (typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(field.Nullable))
						{
							IsPublic = true
						};
				...
				case ResultKind.Void:
					...
					funcModel.Return = _typeParser.Parse<object>().WithNullability(true);
					break;
			}
		...
		private void BuildStoredProcedure(DataModel model, StoredProcedure func, ISet<string> defaultSchemas)
		{
			...
			switch (func.Result.Kind)
			{
				...
				case ResultKind.Scalar:
				{
					...
					funcModel.Return = new ReturnParameter(new ParameterModel(paramName, (typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(scalarResult.Nullable), Direction.Out), scalarResult.Type)
					{
						DataType = typeMapping?.dataType,
		...
		private void BuildParameters(IReadOnlyCollection<Parameter> parameters, List<FunctionParameterModel> models)
		{
			foreach (var param in parameters)
			{
				...
				{
					case ParameterDirection.Input: direction = Direction.In; break;
					case ParameterDirection.InputOutput: direction = Direction.Ref; break;
					case ParameterDirection.Output: direction = Direction.Out; break;
					default:
						throw new InvalidOperationException();
				}

				var parameterModel = new ParameterModel(paramName, (typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(param.Nullable), direction)
				{
			...
			IReadOnlyCollection<ResultColumn> columns)
		{
			...
			foreach (var col in columns)
			{
				...
				var property  = new PropertyModel(
					...
					(typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(col.Nullable))
				{
					IsPublic = true,
					HasSetter = true,
					IsDefault = true,
		...
		private DataContextModel BuildDataContext()
		{
			...
			if (_codegenSettings.BaseContextClass != null)
				dataContextClass.BaseType = _typeParser.Parse(_codegenSettings.BaseContextClass, false);
			else
				dataContextClass.BaseType = _typeParser.Parse<DataConnection>();
			...
			IType? baseType)
		{
			...
			contextPropertyName = contextPropertyName != null
				//? _languageServices.NormalizeIdentifier(contextPropertyName)
				? contextPropertyName
			...
			className = className != null
				//? _languageServices.NormalizeIdentifier(className)
				? className
				: _namingServices.NormalizeIdentifier(
			...
			if (!hasCustomClassName && !_contextSettings.GenerateSchemaAsType && isNonDefaultSchema)
				//className = _languageServices.NormalizeIdentifier(table.Name.Schema + "_" + className);
			...
			foreach (var column in table.Columns)
			{
				...
				var propertyType = (typeMapping?.clrType ?? _typeParser.Parse<object>()).WithNullability(column.Nullable);
